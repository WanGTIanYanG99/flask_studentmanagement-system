...
from flask import app, render_template, request, Flask, url_for, session
import sqlite3
...
def updateuserinfo():
    try:
        ...
        with sqlite3.connect("database.db") as con:
            cur = con.cursor()
            # print(id, username, depart, idnumber, sex)
            cur.execute("update userinfo set username=?, depart=?, idnumber=?, sex=? where id =?",(username,depart,idnumber,sex, id))
            con.commit()
    ...
    if request.method == 'POST':
        try:
            ...
            with sqlite3.connect("database.db") as con:
                ...
                print(role)
                if role != []:
                    msg = str(id)+ 'login successfully!'
                    return redirect(url_for('home'))
                else:
                    msg = 'wrong with password or username'
                    return render_template('login.html', msg=msg)
        except:
            print('error happens when hanlding login')
        finally:
            con.close()
    ...
    if request.method == 'POST':
        try:
            ...
            with sqlite3.connect("database.db") as con:
                cur = con.cursor()
                print(id,courseid,coursename,teachername,score)
                cur.execute("insert into course(id,courseid,coursename,teachername,score) values(?,?,?,?,?)",(id,courseid,coursename,teachername,score))
                con.commit()
...
def analize():
    ...
    con.row_factory = sqlite3.Row
    cur = con.cursor()
    cur.execute("select course.id as id,userinfo.username as username,userinfo.sex as sex, sum(course.score) as sum,count(course.id) as count,sum(course.score)/count(course.id) as average from course left join userinfo on userinfo.id = course.id left join user on user.id = userinfo.id where role='学生' group by course.id order by sum(course.score)/count(course.id) desc")
    rows = cur.fetchall();
